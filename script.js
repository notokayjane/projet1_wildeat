...
// fonction pour conserver les filtres
function updateCards() {
  ...
  if (searchInput.value != "") {
    ...
    filteredRestaurants = filteredRestaurants.filter((restaurant) => {
      return restaurant.restaurantName.toLowerCase().includes(research);
    });

    /*  if (checkFrancais.checked) {
        filteredRestaurants = filteredRestaurants.filter((restaurant) => {
          return restaurant.restaurantType === "Français";})
    ...
    /* if (availableCheckbox.checked) {
       filteredRestaurants = filteredRestaurants.filter((restaurant) => {
         return restaurant.available === true;
       });
     }
   /*
...
// afin de faire un tri des filtres actif

const filterSelect = [];
const checkbox = document.getElementsByName("filtres");
function checkboxFilter (checkbox){
function updateCheckbox() {
  if (checkbox.checked === true) {
    filterSelect.push(checkbox.value)
  } else {
    filterSelect.splice(filterSelect.indexOf(checkbox.value),1)
  }
